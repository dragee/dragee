...
const Dragee = { util, Draggable,  Target, Event }//todo remove after refactore
...
Scope.prototype.onEnd = function(draggable) {
  const shotTargets = this.targets.filter(function(target) {
...
function scopeFactory(parentElement, draggableElements, targetElements, options={}) {
  ...
  const targets = targetElements.map(function(element) {
